...
// DONE: Implement the functionality of `count_for` but with an iterator instead of a `for` loop.
...
// DONE: Implement the functionality of `count_collection_for` but with an iterator instead of a `for` loop.
